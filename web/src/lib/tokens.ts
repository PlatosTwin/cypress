 ...
 *
 * Rendered from Cypress/DesignSystem/Tokens/*.swift by web/scripts/export-tokens.mjs
 * (\`npm run tokens\`). web/test/tokens.test.ts re-runs that render against the Swift and fails
 * when this file and its source disagree, so an edit here is reverted by the next test run.
 *
