...
// is gone and there is nothing left to revoke with. A failure is logged and the deletion proceeds:
// the alternative is refusing to delete somebody's account because a third party is having an
// outage, which breaks R3's promise to keep Apple's — and the row is not lost, because the token is
// only cleared once a revocation has actually succeeded.
func (s *Server) deleteMe(w http.ResponseWriter, r *http.Request, who caller) error {
	...
	if user.AppleRefreshToken != "" {
		if revokeErr := s.Apple.Revoke(r.Context(), user.AppleRefreshToken); revokeErr != nil {
			s.Log.Error("Apple revocation failed; deletion proceeding",
				"user_id", user.ID, "cause", revokeErr)
		} else if clearErr := s.Store.ClearAppleRefreshToken(r.Context(), user.ID); clearErr != nil {
			s.Log.Error("clearing Apple refresh token", "user_id", user.ID, "cause", clearErr)
		}
	}

	report, err := s.Store.DeleteAccount(r.Context(), user.ID, choice, request.PendingClientUUIDs)
	if err != nil {
		return apierr.Wrap(apierr.ServerError, "Something went wrong on our end.", err)
	}
