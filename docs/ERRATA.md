...
5. Lightness descends A→D in light and ascends in dark, so the rank is faintly legible as weight. Not
   load-bearing; the glyph is.

The tightest pair in the set is slot B after dark against the GPS dot, ΔE 0.110, and it is separated
by three further channels: the dot carries no glyph, it is the only mark with an 8 pt halo, and it is
drawn *under* every tree (`zPriority = .min`).
...
Three properties make it unconfusable with E149's species colours, and they are properties by
construction rather than by taste:

1. **It is achromatic.** The two ring colours carry OKLCh chroma ≤ 0.025; every species slot carries
   ≥ 0.080. There is no overlap and no near miss, and
...
E149 just spent its whole design keeping countable — 24 entries to 48, for a state that is true of one
