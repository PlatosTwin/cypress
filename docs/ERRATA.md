...
### E175 — Four trees in five have no planting date, so a year filter is mostly a claim about the city's paperwork

**Amended the same day, by the mechanism this entry created.** The heading read *three* trees in four
when it was written, measured against a seed holding San Francisco alone. San Jose landed hours later
(E176) and the coverage test this entry is about went red on the merge — which is exactly what it was
written to do. San Jose publishes a planting date for **222 of its 52,788 rows (0.42 %)** against San
...
asserted against the shipped seed rather than remembered from the day it was true. **The counts below
are the San-Francisco-only figures as originally measured; they are left as they were because they are
what the design decision was made against, and because SF's own coverage did not change.**
...
### E185 — The care log's photo/note well was drawn and inert, and everything under it worked (task #147)
...
### E186 — The flowering tree the app had no word for (task #151)
...
RULINGS R35: observed states are the observer's report,
...
### E187 — The 44 pt clear control that measures 43.99999999999999 pt on an iPhone 16e

*Found in passing by the #151 branch; the file it implicates is a Map file belonging to another live branch, so this records rather than fixes.*
...
### E195 — The blank-screenshot guard failed open in more ways than the one on record (task #93)
...
### E196 — The AX5 sweep's defect inventory (task #144)

*One entry for the sweep's findings; each finding below is a candidate ticket for the next
round, not a fix in this one.*
...
### E197 — Screen 05's optional well is wired; E25's waiting state ends (task #169)
...
### E198 — The favorite that reverted only on the owner's phone (task #174; supersedes the readings of #139, #153, #167)

**The defect.** Tapping Favorite on a tree profile flashed dark green for about half a second
and went back to white — on the owner's physical iPhone, on every build since the feature
shipped, through four fixes, and never once on a fresh simulator.

**The root cause.** `LocalAPI.signOut()` keeps `device.user_id` — deliberately, so the account
can be resumed — but `LocalAPI.sync` ran `adoptRowsWrittenAfterTheClaim` after **every batch
that applied anything**, gated only on that claim row existing. On a device that had ever
signed in and signed out (E124 signs the owner in at the third visit save; the You tab's Sign
out is one tap), the sequence on every favorite tap was:
...
The same sweep silently re-attributed every device-owned visit, photo, reminder and photo vote
...
E157 family: sign-out promises "attribution goes back to the device," and the post-batch
re-claim was quietly un-promising it one batch later.

**Why four fixes missed it.**
- **#139** joined the write and the read through the real store — on a store with no claim row.
- **#153** and **#167** each widened the *read* (outbox `pendingFavoriteState` first, applied
  rows second). But the toggle here syncs successfully and lands `done` — `done` is skipped by
  design, and the applied row itself was being moved out from under every read there is.
...
- Every verification ran on a fresh simulator, which has no `device.user_id` row. The one row
  of device history that mattered was the one no fixture wrote. Screen 17 could not help the
  owner either: the sync genuinely succeeded, so there was no failed item to show a reason for.

**The fix.** `adoptRowsWrittenAfterTheClaim` now requires the claimed account to be the
currently signed-in one (`claimedUser == self.userID`, plus `userID != nil`). The straddle the
method exists for — queued before sign-in, drained after — has the person signed in at drain
time, so that case is unchanged; a tail drained while signed out stays the device's and is
adopted by the first re-claim after the account is resumed, which is when the promise "your
work comes with you" is back in force. Devices already in the stale state need no data repair.

**The test.** `FavoriteRoundTripTests.aFavoriteSavedAfterSignOutSurvivesTheStaleClaim` — real
seed, real store: claim, sign out, tap, and assert the applied row still answers the device on
...
**The fix** (RULINGS R42): the shell now keeps the grabber's promise —
...
### E202 — The UI suite inherits the device's state, and two kinds of leftovers read as broken screens (task #157)

*Both halves found closing #157 (city downloads), 2026-08-02, on iPhone 16 Plus
`24D1629F-9FA8-4E3D-812E-F6BC85C9E668`. Neither is a defect in #157's diff; both are false reds
that cost an afternoon each if the next agent believes the failure text.*
...
### E204 — A tree reference draws the blurry placeholder even when the tree has photographs (task #176)
...
### E205 — R41 applied: the map's filter status line is gone, and R43 §5 goes with it (tasks #178, #179, #180)

*Measured and applied on branch `p1/map-filters`, 2026-08-02.*
...
### E206 — 12,518 vacant sites is a San-Francisco-only number, and four documents still quote it (tasks #178, #179)

*Measured on the shipped two-city seed, 2026-08-02.*
...
#### What was done at merge

Re-measured independently against the shipped seed before anything was written:
`SELECT COUNT(*), SUM(status='vacant_site') FROM trees` → **24,200 of 198,625 (12.18%)**, confirming
the branch's figure.

`docs/ROADMAP.md` §1 and `docs/RULINGS.md` R7 and R29 were corrected, since both are read as
current. The two investigation documents were **deliberately left alone**: they are dated records of
what was true when they were written, and rewriting a measurement inside an investigation would
destroy the evidence rather than update it. Where the old figure is load-bearing to a decision's
reasoning, it is kept beside the new one rather than replaced, so the ruling still reads as the
...
### E214 — #106's premise: San Francisco Rec & Park publishes no tree inventory, and Golden Gate Park has no source to populate from (task #106)
...
### E216 — The UI suite inherits the simulator's *location*, and a fix on a treeless block reads as a broken map
...
### E226 — A PR's check list can never test the `ships` predicate, and `release: SKIPPED` there means nothing (task #215)
