...
## 10b. Species curation, after RULING D20

**The species map was inert, and finding that out is the whole of this section.**
`Fixtures/*_species_map.csv` is an OUTPUT — `build_seed.py` regenerates it on every run and its own
comment says "a correction belongs in the tables at the top of this script, not in the CSV". The
variable that reads it (`map_path`) is assigned and never used. So the R0–R4 cascade of §4 produced
a report and changed nothing in the build: `species.uuid = uuid5(NS_SPECIES, <lowercased scientific
name>)` is a pure function of the name the ADAPTER emits, and `Platanus x acerifolia` therefore
minted its own species row beside the corpus's `Platanus acerifolia`.

Which means the 85.99% of §4 was measuring **overlap between NYC's flora and California's**, and no
amount of editing a CSV could move it. What a species row actually lacked was CONTENT:

| | before curation | after |
|---|---:|---:|
| NYC rows whose species carries `family` | 52.49% | **99.01%** |
| NYC rows whose species carries `leaf_retention` | 52.12% | **92.50%** |
| SF/SJ rows, for comparison | 93.43% / 92.63% | unchanged |

**`Fixtures/species/nyc_species.yaml` is the third species fixture**, 503 entries, generated by
`Tools/build_nyc_species_content.py` and shaped exactly like `leaf_retention.yaml`. Authorities are
the corpus's own, and no new one was introduced:

| field | authority | sourced | method |
|---|---|---:|---|
| `family` | GBIF Backbone Taxonomy, CC BY 4.0 | 481 of 503 | `gbif_match_exact`, `gbif_match_genus_rank` (a bare genus), `gbif_match_species_level` (a cultivar's parent binomial) |
| `leaf_retention` | Cal Poly SelecTree, `foliage_type` | 374 of 503 | `selectree_name_exact`, `selectree_species_level`, `selectree_synonym_other_taxa` |

**21 entries carry neither and are `null`**, which is the corpus's own convention for "no source
answered" — 66 of `leaf_retention.yaml`'s 577 entries are the same. A FUZZY GBIF match is discarded
rather than recorded: it is a guess with a confidence score attached, and this file may not contain
guesses. No genus-uniformity inference was made either, although SOURCES.md's prose mentions the
technique — **no citation in `leaf_retention.yaml` actually records such a method**, so there was
nothing to follow.

Per borough, every pack clears the gate on both fields:

| pack | `family` | `leaf_retention` |
|---|---:|---:|
| Manhattan | 99.21% | 92.62% |
| Bronx | 98.91% | **90.27%** |
| Brooklyn | 99.07% | 94.17% |
| Queens | 98.83% | 92.29% |
| Staten Island | 99.28% | 92.22% |

Read against the grown corpus, the §4 mapping number becomes **99.41%** — but that reading is
close to tautological, because every NYC name minted its own species row. The 0.59% remainder is
`Unknown - Unknown` (5,238 rows) and 21 blank strings, which can never carry a species at all
(RULINGS R18). **92.50% is the number that means something**, because it is the one that moved.
