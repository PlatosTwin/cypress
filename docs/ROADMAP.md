...
| **W-B** | The three portable assets | Design tokens exported from the Swift declarations to CSS custom properties, with a test that the export still matches its source; the domain rules (vitality rubric, `Quantity`, the 25 m grid, growth-charting eligibility, ID spaces) re-derived in TypeScript against ported Swift test cases; a read layer that opens a published city pack through the same schema the phone uses. **The domain rules third is SHIPPED** by `web/domain-rules`: `web/src/lib/` holds all five, each checked against its original parsed at run time (the Swift, `Tools/inventory_contract.py`, and the two distilled rubric tables) rather than against a transcription, with the Swift's own cases ported. The ID spaces turned out to have **no Swift declaration** — `Tree.idSpace` is an opaque string and the registry lives only in `Tools/inventory_contract.py`. Two findings came out of it: the 25 m grid is not idempotent and the read path applies it twice (`docs/errata-pending/public-photo-grid-is-not-idempotent.md`), and there is no dedicated `Quantity` suite in `CypressTests` — the conversion table was unasserted until now. Design-token export and the pack read layer are still open. |
