...
| ~~**W-C**~~ | ~~W1 · Public tree page~~ | ~~The page renders from a real pack at `/‹id-space›/tree/‹uuid›`, matching the `SCREENS.md` §W1 transcription, with the OpenGraph image its caption specifies rendered from the same ingredients.~~ **SHIPPED** by `web/w1-tree-page`, for the city-record half. Server-rendered from a mounted pack; verified against the pinned seed (sha256 and `count(*)` both checked) on a San Francisco tree, a San Jose vacant site, and on a synthetic `us-ny-nyc` pack. **Seven of §W1's eleven elements are omitted rather than stubbed** — height, the taped DBH reading, the foliage strip, its photo caption, the recent-visits panel, `Sign in` (owner decision 7) and `Open in the app` (no host resolves) — and four labels say less than the mock does; the element-by-element account and the four deviations are in `docs/errata-pending/web-w1-tree-page.md`. **The OpenGraph image is an SVG**, which the major social platforms do not render: making it a PNG needs a rasterizer, which is a dependency decision with an owner — chip backlog 24. New York's disclaimer now follows its data onto the page (R36 consequence (b), R78 rulings 2 and 3). **This row is struck for the pack half only.** W-G's `GET /api/v1/public/trees/{id}` merged (#163) while W-C was in flight and **this page does not call it yet**; the three elements it would restore — height, the contributed trunk DBH with its method badge, and the beloved state — are chip backlog **26**. Two questions are open for the owner in `docs/rulings-pending/w1-authored-copy.md`: two authored strings, and whether this row should have stayed open until W1 draws what W-G publishes. |
...
24. **Decide whether the OpenGraph card is rasterized, and by what.** W-C ships
...
26. **Have W1 call the public community read it was ordered after.** W-G shipped
    `GET /api/v1/public/trees/{id}` (#163) and W-C shipped the page without calling it, so §W1's fact
    column is still the pack half alone. Three elements come back with the call, and W-G's ruling
    already decides their shape: the **latest live height** (value, entered unit, method, month), the
    **latest live trunk DBH** in the same shape — which is §W1's `64 cm` + `taped`, distinct from the
    city's published bucket the page draws today — and the **beloved** state with its count above the
    floor. `Status` stays on the pack's lifecycle value: W-G removed vitality from the endpoint
    because it has no takedown route, and the roadmap's W-G note says which three guards come back
    **before** that projection, not after. Nothing in W1 resists this — the contributed rows are
    omitted rather than stubbed, and `facts()` builds a list.
