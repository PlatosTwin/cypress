...
- `RemoteAPI` — a stub today. When the Go service in `server/` is wired up it implements the same
  protocol against `/api/v1`, and `LocalAPI` becomes the offline cache behind it. (Go, not Fastify:
  the row above, ratified as RULINGS **R72**.)
