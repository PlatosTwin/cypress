...
**2026-08-06. For the owner to pick from. Nothing in this document shipped as written — it was a
camera rig, not an implementation.**

> **Landed on main 2026-08-08, unchanged except for this note and the closing paragraph.** It was
> written on the throwaway branch `design/14-proposals`, which no longer exists, and RULINGS R68 and
> ERRATA E249 both cite it by path — a numbered entry pointing at a document that lived on one
> laptop. The owner picked 17b, 10a + unclamp, and 12a; R68 records the decisions and E249 the two
> findings this sweep turned up on the way. What is preserved here is the evidence behind them: the
> renders, the measured ratios, and the candidates that were **not** chosen. Read it as the argument
> R68 concluded, not as current design.
...
The rig itself was a `DesignProposalVariant` enum under `Cypress/App/`, a `DesignProposalShots` UI
test, and five call-site patches — all of it on the branch this was written on, none of it ever
merged. When a proposal is picked it is implemented in the token layer instead, with its registry
row, its gallery entry and its `ContrastTests` pin. That is what shipped as R68.
