...
//  outbox *first*, and only then attempted against the API." The favorite is durable before
//  anything is attempted, and the drain that follows is best effort — a drain that fails has not
//  lost the favorite, so it is not a failed save.
//
...
//  the measurement has a keypad. The favorite has none of that — screen 03 hands out a tree id and
...
/// What a favorite save produced, for a caller that wants to know what was queued.
...
public enum FavoriteOutboxWriter {
    ...
    ///   with a tombstone (BUILD-PLAN §4 and §6), so replaying "favorited" twice leaves one
    ///   favorite rather than flipping it back off.
    ...
    ///   control save one favorite. It moves with each flip, which is what makes the store's replay
    ///   guard (`WHERE client_uuid <> excluded.client_uuid`) tell a replay from a real un-favorite.
