...
//  species-to-color function: 569 hues is a color wheel, and a reader asked to distinguish the
...
//  naming. Matching needs colors that are unique among the ones on screen right now; it does not
//  need a color that means *Platanus* forever. So this is a **slot**, and `MapSpeciesPalette`
//  hands the four slots out to the four most common species in the current viewport.
//
//  Measured against the shipped seed, over ten neighborhoods at the zooms a reader browses at
...
//  glyph — and Canopy green is deliberately **not** one of the four slot colors. Green is the
...
//  RULINGS R8 settled this for C23 and the sentence generalizes: "a chart that distinguishes its
//  lines only by hue is unreadable to a color-blind reader at *any* contrast ratio, so the
//  redundant encoding is owed regardless". A map is a data encoding on the same terms. **Each slot
//  therefore carries a glyph as well as a hue**, drawn in the ring color inside the pin, and the
...
//  (`MapPin.Kind.removed`) is a gray 16 pt pin with a *horizontal* bar across it. Two bars at right
//  angles on two different colors at two different sizes cannot be read for each other; two
//  parallel ones could.
//
...
/// One of the four color slots a viewport can hand out. **Ordinal, not semantic** — slot `a` does
...
/// becomes the busiest, because a color that changed under the reader's eye would cost more than an
...
enum MapSpeciesSlot: String, CaseIterable, Hashable, Identifiable, Sendable {
    ...
    /// chip, where "plum, dot" is how a color-blind or blind reader ties the chip to the pins.
...
/// The glyph inside a species-colored pin, and the same glyph inside its legend chip.
///
...
/// mark off-center inside the dot.
